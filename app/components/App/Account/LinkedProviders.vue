<template>
  <UCard :ui="{ body: 'p-0 sm:p-0' }">
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <UButton label="Link New Account" color="neutral" @click="linkAccountModal = true" />
      </div>
      <p class="mt-1 text-sm text-neutral-500">Link a new OAuth provider to your account.</p>
    </template>
    <ul v-if="linkedAccounts" class="divide-y divide-neutral-200 dark:divide-white/10">
      <li v-for="account in linkedAccounts" :key="account.id" class="flex items-center gap-3 p-4">
        ...
        <div class="flex-1">
          ...
          <p class="text-xs text-neutral-500">
            Connected on
            {{ account.createdAt ? useDateFormat(account.createdAt, 'MMM D, YYYY').value : 'Unknown date' }}
          </p>
        </div>
        <UButton
          icon="i-lucide-trash"
  ...
  <UModal v-model:open="linkAccountModal" title="Link Account" description="Link an account to your account.">
    <template #body>
      <div class="space-y-2">
...
import type { OAuthAccount } from '@@/types/database';

const linkAccountModal = ref(false);
const toast = useToast();
const loading = ref(false);

const { data: linkedAccounts } = await useFetch<OAuthAccount[]>('/api/me/linked-accounts', {
  key: 'linked-accounts'
});
