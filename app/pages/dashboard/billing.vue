<template>
  <AppPageContainer title="Billing" description="Manage your billing information and subscription plans.">
    <div
            ...
            <template v-if="activeSubscription">
              ...
              <div class="flex flex-wrap items-center gap-3">
                <div class="flex items-center gap-2">
                  <span class="text-xl font-semibold">{{ formatPrice(currentPlan.amount) }}</span>
                ...
                <span class="text-sm text-neutral-500">
                  ...
                  {{ useDateFormat(currentPlan.cancelAt || currentPlan.currentPeriodEnd, 'MMM DD, YYYY') }}
                </span>
              </div>
            </template>
            <template v-else>
              <h3 class="text-lg font-medium">You are on the <span class="font-bold">Free</span> plan</h3>
...
const { data: activeSubscription } = await useFetch<ExpandedSubscription>('/api/stripe/subscription');
...
const currentPlan = computed<BillingPlan>(() => {
  ...
  const plan = plans.value.find((p) => p.id === activeSubscription.value?.priceId);
