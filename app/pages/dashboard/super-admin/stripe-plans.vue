<template>
  <AppPageContainer title="Stripe Plans">
    <template #actions>
      <UButton
    ...
    <div class="space-y-4">
      <SuperAdminPlanCard v-for="product in uniqueProducts" :key="product.id" :title="product.name">
        <template #actions>
          <UButton
        ...
        <div class="divide-y divide-neutral-100 dark:divide-white/10">
          ...
          >
            ...
            <div class="text-right">
              ...
              <p class="text-xs text-neutral-500 dark:text-white/50">per {{ plan.interval }}</p>
            </div>
          </div>
        </div>
...
const { data: plans, refresh: refreshPlans } = await useFetch('/api/stripe/plans');
