<template>
  <UContainer class="space-y-4 py-12">
    <p class="text-7xl font-bold">Hello World</p>
    <NuxtLink class="block underline" to="/auth/login-email-password"> Login (Email/Password) </NuxtLink>
    <NuxtLink class="block underline" to="/auth/magic-link-login"> Login (Magic Link) </NuxtLink>
    <NuxtLink class="block underline" to="/auth/register"> Register </NuxtLink>
    <NuxtLink class="block underline" to="/dashboard"> Dashboard </NuxtLink>
  </UContainer>
</template>
