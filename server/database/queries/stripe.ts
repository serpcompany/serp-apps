...
export const createStripeProduct = async (payload: InsertProduct) => {
  const product = await useDB().insert(tables.products).values(payload).returning().get();
  return product;
};

export const createStripePrice = async (payload: InsertPrice) => {
  const price = await useDB().insert(tables.prices).values(payload).returning().get();
  return price;
};
...
export const createOrUpdateStripeProduct = async (payload: InsertProduct) => {
  ...
  await useDB().delete(tables.prices).where(eq(tables.prices.productId, payload.id)).run();
  await useDB().delete(tables.products).where(eq(tables.products.id, payload.id)).run();
...
export const createOrUpdateStripePrice = async (payload: InsertPrice) => {
  ...
  await useDB().delete(tables.prices).where(eq(tables.prices.id, payload.id)).run();
