import type { Feedback, InsertFeedback, InsertSubscriber } from '@@/types/database';

export const createFeedback = async (payload: InsertFeedback) => {
  const feedback = await useDB().insert(tables.feedback).values(payload).returning().get();
  return feedback;
};
...
export const updateFeedback = async (id: string, payload: Partial<Feedback>) => {
...
export const deleteFeedback = async (id: string) => {
  const feedback = await useDB().delete(tables.feedback).where(eq(tables.feedback.id, id));
  return feedback;
};
...
export const insertSubscriber = async (payload: InsertSubscriber) => {
  const subscriber = await useDB().insert(tables.subscribers).values(payload).returning().get();
  return subscriber;
};
